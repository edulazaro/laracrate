...
>
    ...
    <div x-show="queue.length > 0" class="mt-6 space-y-3" x-cloak>
        <div class="flex items-center justify-between gap-3">
            <p class="text-[10px] font-mono uppercase tracking-[0.25em] text-fuchsia-400">
            ...
            <div class="flex items-center gap-2">
                ...
                <button x-show="!uploading && pendingCount > 0" type="button" @click="clearQueue()" class="text-[10px] font-mono uppercase tracking-[0.15em] text-fuchsia-400 hover:text-fuchsia-300">{{ __('laracrate::uploader.cancel') }}</button>
                <button x-show="!uploading && pendingCount > 0" type="button" @click="startBatch()" class="inline-flex items-center px-3 h-8 rounded border border-cyan-400 bg-cyan-500/20 text-cyan-300 text-[10px] font-bold uppercase tracking-[0.15em] hover:bg-cyan-500/30 hover:shadow-[0_0_12px_rgba(34,211,238,0.5)] transition-all">{{ __('laracrate::uploader.submit') }}</button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <template x-for="(item, i) in queue" :key="item.id">
                <div class="relative aspect-square rounded overflow-hidden border border-fuchsia-500/40 bg-neutral-900">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center transition-opacity duration-300" :class="{ 'bg-fuchsia-600/40 opacity-100': item.status === 'uploading', 'bg-cyan-500/40 opacity-100': item.status === 'done', 'bg-red-700/60 opacity-100': item.status === 'error', 'opacity-0': item.status === 'fade' || item.status === 'pending' }">
                        <template x-if="item.status === 'uploading'"><svg class="animate-spin h-5 w-5 text-cyan-300" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></template>
                    ...
                    <template x-if="item.status === 'pending'"><button type="button" @click.stop="removeItem(i)" class="absolute top-1 right-1 inline-flex items-center justify-center w-5 h-5 rounded border border-fuchsia-500/40 bg-neutral-900 text-fuchsia-300"><svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M6 6l12 12M6 18L18 6"/></svg></button></template>
