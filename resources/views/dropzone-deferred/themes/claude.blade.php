    ...
    <div x-show="queue.length > 0" class="mt-6 space-y-3" x-cloak>
        <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <button x-show="!uploading && pendingCount > 0" type="button" @click="clearQueue()" class="text-[12px] text-[#6B6560] hover:text-[#1A1A1A] underline">{{ __('laracrate::uploader.cancel') }}</button>
                <button x-show="!uploading && pendingCount > 0" type="button" @click="startBatch()" class="inline-flex items-center px-3 h-8 rounded-md bg-[#C15F3C] text-white text-[13px] font-semibold hover:bg-[#A84D2C] transition-colors">{{ __('laracrate::uploader.submit') }}</button>
            </div>
        </div>
            ...
            <template x-for="(item, i) in queue" :key="item.id">
                <div class="relative aspect-square rounded-md overflow-hidden border border-[#1A1A1A]/[0.08]">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center transition-opacity duration-300"
                         :class="{ 'bg-[#1A1A1A]/40 opacity-100': item.status === 'uploading', 'bg-[#C15F3C]/50 opacity-100': item.status === 'done', 'bg-red-700/50 opacity-100': item.status === 'error', 'opacity-0': item.status === 'fade' || item.status === 'pending' }">
