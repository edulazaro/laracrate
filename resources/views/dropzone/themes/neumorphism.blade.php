...
>
    ...
    <div x-show="queue.length > 0" class="mt-6 space-y-3" x-cloak>
        <div class="flex items-center justify-between">
            <p class="text-xs text-gray-600 italic"><span x-show="uploading">{{ __('laracrate::uploader.uploading') }}</span><span x-show="!uploading && doneCount > 0" x-text="doneCount + ' OK' + (errorCount > 0 ? ' / ' + errorCount + ' err' : '')"></span></p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <template x-for="(item, i) in queue" :key="item.id">
                <div class="relative aspect-square rounded-xl overflow-hidden"
                    ...
                    <div class="absolute inset-0 flex items-center justify-center transition-opacity duration-300" :class="{ 'bg-gray-700/40 opacity-100': item.status === 'uploading' || item.status === 'pending', 'bg-emerald-500/40 opacity-100': item.status === 'done', 'bg-red-500/50 opacity-100': item.status === 'error', 'opacity-0': item.status === 'fade' }">
