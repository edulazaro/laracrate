...
>
    ...
    <div x-show="queue.length > 0" class="mt-4 space-y-2" x-cloak>
        <div class="flex items-center justify-between text-xs text-gray-500">
            <span><span x-show="uploading">{{ str(__('laracrate::uploader.uploading'))->lower() }}</span><span x-show="!uploading && doneCount > 0" x-text="doneCount + ' OK' + (errorCount > 0 ? ' / ' + errorCount + ' err' : '')"></span></span>
        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
            <template x-for="(item, i) in queue" :key="item.id">
                <div class="relative aspect-square overflow-hidden">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center transition-opacity duration-300" :class="{ 'bg-blue-500/30 opacity-100': item.status === 'uploading' || item.status === 'pending', 'bg-blue-500/20 opacity-100': item.status === 'done', 'bg-red-500/40 opacity-100': item.status === 'error', 'opacity-0': item.status === 'fade' }">
                        <template x-if="item.status === 'uploading' || item.status === 'pending'"><svg class="animate-spin h-4 w-4 text-blue-700" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></template>
