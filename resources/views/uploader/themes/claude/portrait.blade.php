...
>
    @if($file)
        <div
            ...
            class="rounded-[10px] border border-[#1A1A1A]/[0.08] border-t-2 border-t-[#C15F3C] bg-[#FAF9F5] shadow-[0_8px_24px_rgba(26,26,26,0.06),0_2px_6px_rgba(26,26,26,0.04)] overflow-hidden">
            <div class="aspect-square bg-white border-b border-[#1A1A1A]/[0.08] relative">
                ...
                @endif
                <div x-show="over" x-cloak class="absolute inset-0 bg-[#1A1A1A]/70 flex items-center justify-center">
            ...
            <div class="p-3">
                ...
                <div class="mt-3 flex gap-2">
                    <button type="button" @click="$refs.input.click()"
                        class="flex-1 inline-flex items-center justify-center h-10 rounded-md bg-[#C15F3C] text-white text-[14px] font-semibold hover:bg-[#A84D2C] transition-colors">
                        Reemplazar
                    </button>
                    <button type="button" wire:click="delete" wire:confirm="¿Borrar este archivo?" title="Borrar"
                        class="inline-flex items-center justify-center w-10 h-10 rounded-md border border-[#1A1A1A]/[0.08] bg-white text-[#6B6560] hover:bg-red-50 hover:text-red-700 hover:border-red-200 transition-colors">
    ...
    @else
        <div
            ...
            <p class="mt-3 text-[12px] text-[#8E8881]">máx {{ number_format($maxSizeKb / 1024, 1) }} MB</p>
            <input type="file" x-ref="input" wire:model="upload" accept="{{ $acceptAttr }}" class="hidden" />
