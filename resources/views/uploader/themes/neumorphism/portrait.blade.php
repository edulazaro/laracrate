...
>
    @if($file)
        <div
            ...
            class="rounded-2xl overflow-hidden p-3"
            ...
            <div class="aspect-square rounded-xl relative overflow-hidden"
                ...
                @endif
                <div x-show="over" x-cloak class="absolute inset-0 bg-gray-700/30 backdrop-blur-sm flex items-center justify-center">
            ...
            <div class="pt-3">
                ...
                @if($state === 'pending' || $state === 'processing')
                    <p class="text-xs text-gray-500">{{ __('laracrate::uploader.processing_dots') }}</p>
                @elseif($state === 'failed')
                    <p class="text-xs text-red-600">{{ __('laracrate::uploader.failed') }}</p>
                @endif
                <div class="mt-3 flex gap-2">
                    <button type="button" @click="$refs.input.click()"
                        class="flex-1 inline-flex items-center justify-center h-10 rounded-xl text-gray-700 text-sm font-semibold transition-all"
    ...
    @else
        <div
            ...
            <p class="text-sm font-medium text-gray-700">{{ __('laracrate::uploader.drag_or_click') }}</p>
