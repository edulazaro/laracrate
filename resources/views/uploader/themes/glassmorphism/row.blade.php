...
>
    @if($file)
        <div class="flex items-center gap-4 rounded-2xl bg-white/30 backdrop-blur-2xl p-4 border border-white/40 shadow-[0_8px_32px_rgba(31,38,135,0.15)]">
            @if($previewUrl)
                <img src="{{ $previewUrl }}" alt="" class="h-16 w-16 rounded-xl object-cover border border-white/50 {{ $roundedClass }}" />
            @else
    ...
    @else
        <div
            ...
            <p class="text-sm font-semibold text-gray-900 drop-shadow-sm">{{ __('laracrate::uploader.drag_or_click_long') }}</p>
            ...
            <div wire:loading wire:target="upload" class="mt-2 text-xs text-purple-700">{{ __('laracrate::uploader.uploading') }}</div>
        </div>
