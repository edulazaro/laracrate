...
>
            ...
            <div class="p-3">
                ...
                @if($state === 'pending' || $state === 'processing')
                    <p class="mt-0.5 text-xs text-gray-400 inline-flex items-center gap-1">
                        <svg class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" stroke-opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10"/></svg>Procesando
                    </p>
                @elseif($state === 'failed')
    ...
    @else
        <div
            ...
            <p class="text-sm text-gray-700 font-medium">Arrastra o haz clic</p>
            <p class="mt-1 text-xs text-gray-500">Máx. {{ number_format($maxSizeKb / 1024, 1) }} MB</p>
            <input type="file" x-ref="input" wire:model="upload" accept="{{ $acceptAttr }}" class="hidden" />
            <div wire:loading wire:target="upload" class="mt-2 text-xs text-gray-500">Subiendo...</div>
        </div>
        @error('upload') <p class="mt-2 text-xs text-red-600">{{ $message }}</p> @enderror
    @endif
</div>
