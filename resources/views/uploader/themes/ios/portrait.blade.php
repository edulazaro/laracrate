<div
    ...
    @if($file)
        <div
            ...
            class="rounded-2xl bg-white/70 backdrop-blur-xl ring-1 ring-black/5 shadow-sm overflow-hidden">
            <div class="aspect-square bg-gray-100 relative">
                ...
                @endif
                <div x-show="over" x-cloak class="absolute inset-0 bg-blue-500/40 backdrop-blur-md flex items-center justify-center">
                    <p class="text-[15px] font-medium text-white">Suelta para reemplazar</p>
                </div>
            </div>
            <div class="p-3">
                ...
                <div class="mt-3 flex gap-2">
                    <button type="button" @click="$refs.input.click()"
                        class="flex-1 inline-flex items-center justify-center h-10 rounded-xl bg-blue-600 text-white text-[15px] font-medium hover:bg-blue-700 transition-colors">
                        Reemplazar
                    </button>
                    <button type="button" wire:click="delete" wire:confirm="Borrar este archivo?"
                        class="rounded-full bg-gray-100 hover:bg-red-100 hover:text-red-600 text-gray-500 w-10 h-10 inline-flex items-center justify-center transition-colors">
    ...
    @else
        <div
            ...
            <p class="text-[15px] font-medium text-gray-900">Arrastra o toca</p>
            <p class="mt-0.5 text-[13px] text-gray-500">Máx. {{ number_format($maxSizeKb / 1024, 1) }} MB</p>
            <input type="file" x-ref="input" wire:model="upload" accept="{{ $acceptAttr }}" class="hidden" />
            <div wire:loading wire:target="upload" class="mt-2 text-[13px] text-blue-600">Subiendo...</div>
        </div>
        @error('upload') <p class="mt-2 text-[13px] text-red-600">{{ $message }}</p> @enderror
    @endif
</div>
