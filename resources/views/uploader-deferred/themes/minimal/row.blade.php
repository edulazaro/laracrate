    ...
    @if($state === 'staged')
        <div class="flex items-center gap-3 border-l-2 border-gray-900 bg-white pl-3 py-2">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-10 w-10 rounded object-cover {{ $roundedClass }}" />
            @endif
            ...
            <button type="button" wire:click="cancel" class="text-xs text-gray-500 hover:text-gray-900 underline">{{ str(__('laracrate::uploader.cancel'))->lower() }}</button>
