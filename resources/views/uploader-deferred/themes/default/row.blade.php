...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-lg border-2 border-blue-300 bg-blue-50/40 p-3">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-16 w-16 rounded-md object-cover border border-gray-200 shrink-0 {{ $roundedClass }}" />
            @else
            ...
            <div class="flex-1 min-w-0">
                <p class="text-xs text-blue-700">{{ __('laracrate::uploader.pending') }}</p>
    ...
    @elseif($file)
        <div class="flex items-center gap-4 rounded-lg border border-gray-200 bg-white p-3">
            @if($previewUrl)
                <img src="{{ $previewUrl }}" alt="" class="h-16 w-16 rounded-md object-cover border border-gray-200 {{ $roundedClass }}" />
            @else
            ...
            <div class="flex-1 min-w-0">
                ...
                @if($state === 'pending' || $state === 'processing')
                    <p class="mt-0.5 text-xs text-gray-400 inline-flex items-center gap-1">
                        ...
                        {{ __('laracrate::uploader.processing_dots') }}
                    </p>
                @elseif($state === 'failed')
    ...
    @else
        <div
            ...
            <p class="text-sm text-gray-700 font-medium">{{ __('laracrate::uploader.select') }}</p>
            <p class="mt-0.5 text-xs text-gray-500">{{ __('laracrate::uploader.drag_or_click') }} · {{ __('laracrate::uploader.max_size_capital', ['size' => number_format($maxSizeKb / 1024, 1)]) }}</p>
            <input type="file" x-ref="input" wire:model="pending" accept="{{ $acceptAttr }}" class="hidden" />
            <div wire:loading wire:target="pending" class="mt-2 text-xs text-gray-500">{{ __('laracrate::uploader.preparing') }}</div>
        </div>
        @error('pending') <p class="mt-2 text-xs text-red-600">{{ $message }}</p> @enderror
    @endif
</div>
