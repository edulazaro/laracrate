...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-xl border border-[#0D0D0D]/30 bg-[#F7F7F8] p-3 shadow-[0_6px_24px_rgba(0,0,0,0.05),0_1px_2px_rgba(0,0,0,0.04)]">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-14 w-14 rounded-lg object-cover border border-[#E5E5E5] shrink-0" />
            @else
                <div class="h-14 w-14 rounded-lg border border-[#E5E5E5] bg-white flex items-center justify-center text-[#8E8EA0] shrink-0">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-[12px] text-[#5D5D5D]">Pendiente de subir</p>
            ...
            <button type="button" wire:click="cancel" title="Cancelar"
                class="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-[#E5E5E5] text-[#5D5D5D] hover:bg-white transition-colors">
            ...
            <button type="button" wire:click="submit" wire:loading.attr="disabled" wire:target="submit"
                class="inline-flex items-center px-4 h-9 rounded-lg bg-[#0D0D0D] text-white text-[13px] font-semibold hover:bg-[#1A1A1A] transition-colors disabled:opacity-60">
    ...
    @else
        <div
            ...
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <p class="text-[15px] text-[#0D0D0D] font-semibold">Seleccionar archivo</p>
                    <p class="text-[13px] text-[#5D5D5D]">arrastra o haz clic · máx {{ number_format($maxSizeKb / 1024, 1) }} MB</p>
                </div>
            </div>
            <span class="shrink-0 inline-flex items-center px-3 h-9 rounded-lg border border-[#E5E5E5] bg-white text-[#0D0D0D] text-[13px] font-semibold hover:border-[#0D0D0D] transition-colors">
                Elegir
            </span>
