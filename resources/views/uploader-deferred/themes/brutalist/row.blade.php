...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-[4px] border border-neutral-900 bg-yellow-100 p-4 shadow-[4px_4px_0_0_rgba(10,10,10,1)]">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-16 w-16 rounded-[3px] border border-neutral-900 object-cover shrink-0 {{ $roundedClass }}" />
            @else
            ...
            <div class="flex-1 min-w-0">
                <p class="text-[10px] font-mono uppercase tracking-[0.2em] text-neutral-900 font-bold">{{ __('laracrate::uploader.pending_short') }}</p>
            ...
            <button type="button" wire:click="submit" wire:loading.attr="disabled" wire:target="submit"
                class="inline-flex items-center px-4 h-9 rounded-[4px] border border-neutral-900 bg-neutral-950 text-white text-xs font-bold uppercase tracking-[0.15em] hover:bg-neutral-800 transition-colors disabled:opacity-60">
    ...
    @elseif($file)
        <div class="flex items-center gap-4 rounded-[4px] border border-neutral-900 bg-white p-4 shadow-[4px_4px_0_0_rgba(10,10,10,1)]">
            @if($previewUrl)
            ...
            <div class="flex-1 min-w-0">
                ...
                @if($state === 'pending' || $state === 'processing')
                    <p class="mt-1 text-[10px] font-mono uppercase tracking-[0.2em] text-neutral-700 inline-flex items-center gap-1">
                        <span class="inline-block w-1.5 h-1.5 bg-neutral-900 animate-pulse"></span>{{ __('laracrate::uploader.processing') }}
                    </p>
                @elseif($state === 'failed')
    ...
    @else
        <div
            ...
            <p class="mt-1 text-sm text-neutral-950 font-bold">{{ __('laracrate::uploader.select') }}</p>
            ...
            <div wire:loading wire:target="pending" class="mt-2 text-[10px] font-mono uppercase tracking-[0.2em] text-neutral-700">{{ __('laracrate::uploader.preparing') }}</div>
        </div>
        @error('pending') <p class="mt-2 text-xs text-red-600 font-mono">{{ $message }}</p> @enderror
    @endif
</div>
