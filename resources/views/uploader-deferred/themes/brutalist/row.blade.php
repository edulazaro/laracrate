...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-[4px] border border-neutral-900 bg-yellow-100 p-4 shadow-[4px_4px_0_0_rgba(10,10,10,1)]">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-16 w-16 rounded-[3px] border border-neutral-900 object-cover shrink-0" />
            @else
    ...
    @elseif($file)
        <div class="flex items-center gap-4 rounded-[4px] border border-neutral-900 bg-white p-4 shadow-[4px_4px_0_0_rgba(10,10,10,1)]">
            @if($previewUrl)
                <img src="{{ $previewUrl }}" alt="" class="h-16 w-16 rounded-[3px] border border-neutral-900 object-cover shrink-0" />
            @else
                <div class="h-16 w-16 rounded-[3px] border border-neutral-900 bg-yellow-200 flex items-center justify-center shrink-0">
                    <svg class="w-6 h-6 text-neutral-900" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                </div>
    ...
    @else
        <div
            ...
            <p class="mt-1 text-sm text-neutral-950 font-bold">Selecciona un archivo</p>
