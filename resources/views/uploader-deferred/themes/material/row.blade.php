...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-md bg-indigo-50 p-3 shadow-md ring-1 ring-indigo-200">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-16 w-16 rounded object-cover" />
            @else
    ...
    @elseif($file)
        <div class="flex items-center gap-4 rounded-md bg-white p-3 shadow-md ring-1 ring-black/5">
            @if($previewUrl)
                <img src="{{ $previewUrl }}" alt="" class="h-16 w-16 rounded object-cover" />
            @else
                <div class="h-16 w-16 rounded bg-indigo-100 flex items-center justify-center text-indigo-600">
                    <svg class="w-6 h-6" viewBox="0 0 24 24" fill="currentColor"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg>
                </div>
    ...
    @else
        <div
            ...
            <div wire:loading wire:target="pending" class="mt-3 text-xs text-indigo-600 font-medium">Preparando...</div>
        </div>
        @error('pending') <p class="mt-2 text-xs text-red-600">{{ $message }}</p> @enderror
    @endif
</div>
