...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-md bg-indigo-50 p-3 shadow-md ring-1 ring-indigo-200">
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-16 w-16 rounded object-cover {{ $roundedClass }}" />
            @else
            ...
            <div class="flex-1 min-w-0">
                <p class="text-[10px] font-medium uppercase tracking-wider text-indigo-700">{{ __('laracrate::uploader.pending_short') }}</p>
            ...
            <button type="button" wire:click="cancel"
                class="rounded-full p-2 text-gray-500 hover:bg-gray-100 transition-colors">
            ...
            <button type="button" wire:click="submit" wire:loading.attr="disabled" wire:target="submit"
                class="inline-flex items-center px-4 h-10 rounded-md bg-indigo-600 text-white text-sm font-bold uppercase tracking-wider shadow-sm hover:bg-indigo-700 hover:shadow-md transition-all disabled:opacity-60">
    ...
    @else
        <div
            ...
            <div wire:loading wire:target="pending" class="mt-3 text-xs text-indigo-600 font-medium">{{ __('laracrate::uploader.preparing') }}</div>
        </div>
        @error('pending') <p class="mt-2 text-xs text-red-600">{{ $message }}</p> @enderror
    @endif
</div>
