...
>
    @if($state === 'staged')
        <div class="flex items-center gap-4 rounded-2xl p-4"
            ...
            @if($pendingPreviewUrl)
                <img src="{{ $pendingPreviewUrl }}" alt="" class="h-16 w-16 rounded-xl object-cover {{ $roundedClass }}" style="box-shadow: inset 4px 4px 8px rgba(163,177,198,0.45), inset -4px -4px 8px rgba(255,255,255,0.7);" />
            @else
            ...
            <div class="flex-1 min-w-0">
                <p class="text-xs text-gray-500 italic">{{ __('laracrate::uploader.pending_short') }}</p>
    ...
    @else
        <div
            ...
            <div wire:loading wire:target="pending" class="mt-2 text-xs text-gray-600">{{ __('laracrate::uploader.preparing') }}</div>
        </div>
        @error('pending') <p class="mt-2 text-xs text-red-600">{{ $message }}</p> @enderror
    @endif
</div>
