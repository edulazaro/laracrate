...
<div x-data="laracrateDropzone({presignUrl:@js(route('laracrate.uploads.presign')),disk:@js($disk),fileableType:@js($fileableType),fileableId:@js($fileableId),collection:@js($collection),maxSizeKb:@js($maxSizeKb),persistQueue:false,autoStart:false,maxFiles:1})" class="w-full">
    ...
    @else
        ...
        <div x-show="queue.length > 0 && queue[0]?.status === 'pending'" x-cloak class="border-l-2 border-amber-500 pl-4 py-3 flex items-center gap-3">
            ...
            <button type="button" @click="startBatch()" x-show="{{ ($hideActions ?? false) ? 'false' : 'true' }}" class="text-xs text-gray-900 hover:text-blue-600 underline font-semibold">{{ __('laracrate::uploader.submit') }}</button>
            <button type="button" @click="removeItem(0)" x-show="{{ ($hideActions ?? false) ? 'false' : 'true' }}" class="text-xs text-gray-400 hover:text-red-600 underline">{{ __('laracrate::uploader.cancel') }}</button>
        </div>
