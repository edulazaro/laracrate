...
{{--
    PoC: UN blade para los 11 temas de dropzone-single-deferred (disposición de
    1 archivo). Tema solo por CSS ([data-wire-theme] + clases .lc-dzs__*),
    comparte variables con el resto de componentes. Sin @if por tema.
    hideActions → is-hide-actions (el CSS oculta las acciones).
--}}
<div
    ...
    class="lc-dzs {{ ($hideActions ?? false) ? 'is-hide-actions' : '' }}"
>
    @if ($existing)
        {{-- Archivo ya subido --}}
    ...
    @else
        ...
        {{-- Pendiente: staged --}}
        <div x-show="queue.length > 0 && queue[0]?.status === 'pending'" x-cloak class="lc-dzs__row">
            <span class="lc-dzs__icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></span>
            <div class="lc-dzs__meta">
                <p class="lc-dzs__sub">{{ __('laracrate::uploader.pending') }}</p>
                <p class="lc-dzs__name" x-text="queue[0]?.name ?? '...'"></p>
            </div>
            <div class="lc-dzs__actions">
                <button type="button" @click="startBatch()" class="lc-dzs__btn">{{ __('laracrate::uploader.submit') }}</button>
                <button type="button" @click="removeItem(0)" class="lc-dzs__action lc-dzs__action--danger" title="{{ __('laracrate::uploader.cancel') }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
        </div>
        ...
        {{-- Subiendo --}}
        <div x-show="queue.length > 0 && queue[0]?.status === 'uploading'" x-cloak class="lc-dzs__row lc-dzs__row--uploading">
